...
export class Client {
  ...
  /**
   * listPipelines returns a list of all the pipeline versions for
  ...
  async listPipelines(
    ...
    while (true) {
      const resp = await this.call<ListPipelinesProto>(
      ...
      pipelines = pipelines.concat(
        resp.pipelines.map(pipeline => {
          ...
          let preSteps: Step[] = [];
          if (preStepsProto.steps !== undefined) {
            preSteps = preSteps.concat(
              preStepsProto.steps.map<Step>((step: StepProto) => ({
          ...
          let postSteps: Step[] = [];
          if (postStepsProto.steps !== undefined) {
            postSteps = postSteps.concat(
              postStepsProto.steps.map<Step>((step: StepProto) => ({
                identifier: step.identifier,
                title: step.title,
                description: step.description,
                condition: step.condition,
          ...
          return {
            identifier: pipeline.pipeline.identifier,
            created: new Date(pipeline.createTime),
      ...
      if (
        resp.nextPageToken === "" ||
        resp.nextPageToken === undefined ||
        resp.nextPageToken === null
      ) {
        break;
      }
 ...
 * @hidden
 */
function paramConfigToParam(
  configs: StepProto["parameterConfigs"]
): Step["parameters"] {
  if (configs === undefined) {
    return undefined;
  }

  const output = Object.keys(configs).reduce<NonNullable<Step["parameters"]>>(
    (out, key) => {
      const config = (configs[key].configs || []).reduce(
        (val, cfg) => {
          if ("setName" in cfg) {
            val["name"] = cfg.setName;
          } else if ("setDefaultValue" in cfg) {
            val["defaultValue"] = cfg.setDefaultValue;
          }
          return val;
        },
        {} as {
 ...
 * @hidden
 */
function constConfigToParam(
  configs: StepProto["constantConfigs"]
): Step["constants"] {
  if (configs === undefined) {
    return undefined;
  }
...
interface StepProto {
  ...
  parameterConfigs?: {
    [name: string]: {
      configs?: ({ setName: string } | { setDefaultValue: string })[];
    };
  };
